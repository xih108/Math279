...
int main()
{
   ...
   n = readpicture(infile,width,height);
   infile.close();
   ...
   for (i = 0; i < width; i++) {
      for(j = 0; j < height; j++) {
         for (k=0; k<m; k++) {
            ...
            if (c >= c1 && c <= c2) {
               Q = (c-c1) / (c2-c1) * static_cast<double>(HC[k+1]- HC[k]) + static_cast<double>(HC[k]);
               n[i][j] = (Q - static_cast<double>(HC[0])) / static_cast<double>(HC[m]-HC[0]);
               break;
            }
...
//   free_matrix(f,width,height);
   ofstream outfile("6.holder.bmp", ios::out | ios::binary);
   writepicture(outfile,n,width,height);
   outfile.close();
   free_matrix(n,width,height);
}
...
// height
   cout << " fskduhfuisdh height = " << height << endl;
