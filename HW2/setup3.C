...
for (i = 0; i < width; i++){
   for (j = 0; j < height; j++){
      for (int k = 0; k<= m; k++){
         if (f[i][j] == c[k]){
            g[i][j] = (CH_f[k] - CH_f[0])*1.0/(CH_f[m] - CH_f[0]);
            cg[k] = g[i][j];
            break;
         }
      }
...
// writing picture
   ofstream outfile("3(b).bmp", ios::out | ios::binary);
   writepicture(outfile,g,width,height);
   outfile.close();

   free_matrix(f,width,height);
   free_matrix(g,width,height);
}
