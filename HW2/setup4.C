...
// if reading in picture
   ifstream infile("holder.bmp", ios::in | ios::binary);
...
for (i = 0; i < width; i++){
   for (j = 0; j < height; j++){
      int k = floor(f[i][j]*(m-1));
      f[i][j] = (CH_f[k] - CH_fmin)*1.0/(CH_fmax - CH_fmin);
      cg[k] = f[i][j];
   }
}
int a = 197;
...
// writing picture
   ofstream outfile("4(d).bmp", ios::out | ios::binary);
...
//YOUR FUNCTIONS HERE
//**************************************************************************
// void histogram(int* c, int* hist, int m){
