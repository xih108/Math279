...
double compute(double **f, int i, int j, int width, int height, double h);
//MAIN BODY HERE
int main()
{
   int width;
...
// if reading in picture
   // ifstream infile("buildings.bmp", ios::in | ios::binary);
...
// if creating picture
   ...
   //    }
   // }
   memset(R, 0, sizeof(R[0][0])*width*height);
   ...
   double h = 0.1;
   double deltat = pow(h, 3)/4;

   double **f_tmp = matrix(width, height);
   int k = 0;
   while (true){
      double diff = 0;
      ...
      for (i = 0; i < width; i++ ){
         for (j = 0; j < height; j++){
            if (R[i][j] > 0){
               f_tmp[i][j] = f[i][j] + deltat * compute(f, i, j, width, height, h);
            }
            else {
               f_tmp[i][j] = f[i][j];
            }
            if (abs(f_tmp[i][j] - f[i][j]) > diff){
               diff = abs(f_tmp[i][j] - f[i][j]);
            }
         }
      }
   ...
   printf("k is %d \n", k);
   // printf("The value at pixel (50,70) is %f \n", f[50][70]);


// writing picture
   ofstream outfile("7_final.bmp", ios::out | ios::binary);
...
double compute(double **f, int i, int j, int width, int height, double h){
   double f_x = (f[min(i+1, width-1)][j] - f[max(i-1,0)][j])/(2*h);
   double f_y = (f[i][min(j+1,height-1)] - f[i][max(j-1, 0)])/(2*h);

   double f_xx = (f[min(i+1, width-1)][j] - 2*f[i][j] + f[max(i-1,0)][j])/(h*h);
   double f_yy = (f[i][min(j+1,height-1)] - 2*f[i][j] + f[i][max(j-1, 0)])/(h*h);
   double f_xy = (f[min(i+1, width-1)][min(j+1,height-1)] - f[min(i+1, width-1)][max(j-1, 0)] - f[max(i-1,0)][min(j+1,height-1)] + f[max(i-1,0)][max(j-1, 0)] ) / (4*h*h);
   double num = f_x*(f_x *f_xx + f_y*f_xy) + f_y*(f_x*f_xy + f_y*f_yy);
   double denum = f_x*f_x + f_y*f_y + h*h;
   
   double f_t = 1/sqrt(denum)*(f_xx + f_yy - num/denum);
   return f_t;
}