...
// cout<<"aaaaa"<<endl;
   ...
   while (true){
      ...
      for (i = 0; i < width; i++ ){
         for (j = 0; j < height; j++){
            if (R[i][j] > 0){
               f_tmp[i][j] = f[i][j] + deltat*( 
                  1/C_compute(f, i, j, width, height, epsilon) * (f[min(i+1, width-1)][j] - f[i][j] + f[i][min(j+1, height - 1)] - f[i][j])
...
#endif
// writing picture
   ofstream outfile("2d_final.bmp", ios::out | ios::binary);
