...
// writing picture
   ofstream outfile("3c_final.bmp", ios::out | ios::binary);
...
double C_compute(double **f, int i, int j, int width, int height, double epsilon){
   double C_ij  = sqrt(pow(f[ min(i + 1, width - 1) ][max(j, 0)] - f[max(i, 0)][max(j, 0)], 2) + pow(f[i][min(j + 1, height - 1)] - f[max(i,0)][max(j,0)], 2) + pow(epsilon, 2));
   return C_ij;
}