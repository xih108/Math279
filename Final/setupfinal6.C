...
void writecolorpicture(ofstream& outfile, double **fr, double **fg, double **fb, 
                       int width, int height);
//   YOUR FUNCTIONS HERE
double heat_flow(double **f,int i, int j,  int width, int height, double h);
//MAIN BODY HERE
int main()
{
   int width;
...
// if reading in colorpicture
   ifstream infile("airplaneblurry.bmp", ios::in | ios::binary);
...
// do all your work you want timed between these two
   ...
   int steps = 75;
   h = 1.0;
   double deltatback = h*h/4.0;
   double deltaforward = 2*deltatback/3;
   int Nback = 1;
   int Nforward = 1;
   ...
   for (int k = 0; k < steps; k++){
      ...
      for (int s = 0; s < Nback; s++){
         for (int i = 0; i < width; i++){
            for (int j = 0; j < height; j++){
               f_tmp[i][j] = f[i][j] - deltatback*heat_flow(f, i, j, width, height, h);
            }
         }
      ...
      for (int s = 0; s < Nforward; s++){
         for (int i = 0; i < width; i++){
            for (int j = 0; j < height; j++){
               f_tmp[i][j] = f[i][j] + deltaforward*heat_flow(f, i, j, width, height, h);
            }
         }
         for (int i = 0; i < width; i++){
            for (int j = 0; j < height; j++){
...
// writing picture
   ofstream outfile("6b2.bmp", ios::out | ios::binary);
...
//**************************************************************************
double heat_flow(double **f,int i, int j,  int width, int height, double h){
   double res = (f[min(i+1, width-1)][j] - 2*f[i][j] + f[max(i-1, 0)][j]) / (h*h) 
            + (f[i][min(j+1, height-1)] - 2*f[i][j] + f[i][max(j-1,0)]) / (h*h);
   return res;
}