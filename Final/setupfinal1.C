...
// if reading in colorpicture
   // ifstream infile("seaturtlecolor.bmp", ios::in | ios::binary);
...
// do all your work you want timed between these two
   ...
   // m + 1 evenly intensities (fmin to fmax)
   for (int k = 0; k < m; k++){
      c[k] = k * (fmax - fmin) / (m-1) + fmin; 
      cout<< c[k]<<endl;
   }
   ...
   for (int k = 0; k < m; k++){
      if ( (1-plight) * width * height <= CH_f[k]){
         clight = c[k];
         break;
      }
   }
   cout << cdark <<" "<<clight<<endl;

   // g(f)
   for (i = 0; i < width; i++){
      for (j = 0; j < height; j++){
         f[i][j] = (f[i][j] - cdark) * 1.0 / (clight-cdark) ;
         if (f[i][j] > 1){
            f[i][j] =  1;
         }
         if (f[i][j] < 0){
...
// writing picture
   ofstream outfile("1c2.bmp", ios::out | ios::binary);
   // writecolorpicture(outfile,fr,fg,fb,width,height);
