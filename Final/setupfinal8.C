...
// if reading in colorpicture
   ifstream infile("plant.bmp", ios::in | ios::binary);
...
// do all your work you want timed between these two
   ...
   while (true){
      ...
      cplus = fplus / countplus;
      cminus = fminus / countminus;

      int count = 0;
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            double change = pow(cplus - f[i][j], 2) - pow(cminus - f[i][j], 2);
            if (phi[i][j] > 0 && -change < 0){
               phi[i][j] = -phi[i][j];
               count += 1;
               continue;
            }
      ...
      cout <<k <<" "<<count<<endl;
      cout <<cminus << " "<<cplus << endl;
   }
 
   cout<< "cminus "<<cminus <<" cplus"<< cplus << endl;
// writing picture
   ofstream outfile("8c.bmp", ios::out | ios::binary);
