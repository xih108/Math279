...
void writecolorpicture(ofstream& outfile, double **fr, double **fg, double **fb, 
                       int width, int height);
//   YOUR FUNCTIONS HERE
double compute(double **f, int i, int j, int width, int height, double h, double epsilon);
double gradient(double **f, int i, int j, int width, int height, double h);
//MAIN BODY HERE
...
// if reading in colorpicture
   ifstream infile("tuna.bmp", ios::in | ios::binary);
...
// do all your work you want timed between these two
   ...
   h = 1.0;
   double deltat = h*h / 4.0;
   double epsilon = pow(10, -11);
   double xcenter = width/2.0 + 5;
   ...
   for (i = 0; i < width; i++){
      for (j = 0; j < height; j++){
         phi[i][j] = sqrt(pow(i - xcenter,2)/(a*a) + pow(j - ycenter,2) /(b*b)) - 1;
      }
   }

   double **phi_tmp = matrix(width, height);

   for (int k = 0; k < steps; k++){
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            phi_tmp[i][j] = phi[i][j] + deltat * 1/(1+K*gradient(f, i, j, width, height, h)) * compute(phi, i, j, width, height, h, epsilon);
         }
      }
   ...
   // cout<< "The value at pixel (64,64) is " <<f[64][64] << endl;
// writing picture
   ofstream outfile("7curve1.bmp", ios::out | ios::binary);
   // writepicture(outfile,f,width,height);
   writecurveinpicture(outfile,f, phi, width,height);
   outfile.close();
...
double compute(double **f, int i, int j, int width, int height, double h, double epsilon){
   ...
   double denum = f_x*f_x + f_y*f_y + epsilon * epsilon;
   
   double f_t = f_xx + f_yy - num/denum;
   return f_t;
}
