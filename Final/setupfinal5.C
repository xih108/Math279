...
void writecolorpicture(ofstream& outfile, double **fr, double **fg, double **fb, 
                       int width, int height);
//   YOUR FUNCTIONS HERE
double compute(double **f, int i, int j, int width, int height, double h, double delta);
...
// do all your work you want timed between these two
   ...
   double deltat = h*h/(4*wr + wf*h*h/2);
   int numsteps = 200;
   double **fnoisy = matrix(width, height);
   ...
   for ( k = 0; k < numsteps; k++){
      ...
      for (i = 0; i < width; i++ ){
         for (j = 0; j < height; j++){
            double ft = wr*compute(f, i, j, width, height, h, epsilon) - wf*(f[i][j] - fnoisy[i][j]);
            f_tmp[i][j] = f[i][j] + deltat * ft;
         }
      }
...
// writing picture
   ofstream outfile("5d2.bmp", ios::out | ios::binary);
...
double compute(double **f, int i, int j, int width, int height, double h, double delta){
   ...
   double denum = f_x*f_x + f_y*f_y + delta*delta;
