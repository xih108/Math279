...
// if reading in colorpicture
   ifstream infile("honeycreepercolor.bmp", ios::in | ios::binary);
...
// do all your work you want timed between these two
   ...
   for (int color = 0; color < 3; color ++){
      ...
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            int k = ceil((f[i][j] - fmin)/(fmax-fmin) *(m-1));
      ...
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            ...
            int k = floor((f[i][j] - fmin)*(m-1)/(fmax-fmin));
            //  cout<<k<<endl;
            //interpolant
            if (k < m-1) {
               double t = (cg[k+1] - cg[k])* (f[i][j] - c[k]) / (c[k+1] - c[k])  + cg[k];
               f[i][j] = t;
            }
         }
      }
   ...
   cout<< "The value at pixel (64,64) is (R: " <<fr[64][64]<<", G: "<<fg[64][64]<<", B: "<<fb[64][64]<<")" << endl;
// writing picture
   ofstream outfile("2c2.bmp", ios::out | ios::binary);
