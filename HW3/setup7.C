...
//   YOUR FUNCTIONS HERE
double cubic(double* p, double x);
double Lagrange(double** f, int width, int height, double x, double y, int h);
//MAIN BODY HERE
int main()
{
   int width;
...
// if creating picture
   ...
   for (int r = 0; r < twidth; r++){
      for (int s = 0; s < theight; s++){
         ...
         // // cout<<i<<"&&&& "<<j<<endl;
         // double horizontal[] = {0,0,0,0};
         // for (int k = 0; k < 4; k++){
         //    int x = i + k - 1;
         //    double vertical[] = {0,0,0,0};
         //    for (int t = 0; t < 4; t++){
         //       int y = j + t - 1;
         //       vertical[t] = f[min(max(x,0),width-1)][min(max(y,0),height-1)];
         //    }
         //    horizontal[k] = cubic(vertical, y_s - j);
         // }
         
         g[r][s] = Lagrange(f, width, height, x_r, y_s, h);
         // cout<<r<<", "<<s<<": "<<g[r][s]<<endl;
      }
   }
   // printf("The intensity at pixel (242,127) is %f", g[242][127]);



// writing picture
   ofstream outfile("7c.bmp", ios::out | ios::binary);
   writepicture(outfile,g,twidth,theight);
   outfile.close();

   free_matrix(f,width,height);
   free_matrix(g,twidth,theight);
}
...
//**************************************************************************
double cubic(double* p, double x){
   double i1 = (-1/2.0*p[0] + 3/2.0*p[1] - 3/2.0*p[2] + 1/2.0*p[3])* x * x * x;
   double i2 = (p[0] - 5/2.0*p[1] + 2.0*p[2] - 1/2.0*p[3]) * x * x;
   double i3 = (-1/2.0*p[0] + 1/2.0*p[2])* x + p[1];
   return i1+i2+i3;
}
...
double Lagrange(double** f, int width, int height, double x, double y, int h){
   ...
   double res = 0;
   for (int k = 0; k < width; k+=h){
         double Pk = horizontal[k];
