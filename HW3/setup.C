...
// if reading in picture
   ifstream infile("rose.bmp", ios::in | ios::binary);
   ...
   for (int r = 0; r < twidth; r++){
      for (int s = 0; s < theight; s++){
         ...
         for (int k = 0; k < 4; k++){
            ...
            horizontal[k] = cubic(vertical, y_s - (j-1));
         }
         
         g[r][s] = cubic(horizontal, x_r - (i-1));
         // <<g[r][s]<<"&&&& "<<endl;
      }
   }
...
// writing picture
   ofstream outfile("4_a.bmp", ios::out | ios::binary);
