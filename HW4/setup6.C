...
// if creating picture
   ...
   int C [width][height];
   memset(C, 0, sizeof(C[0][0])*width*height);
   ...
   while(true){
      ...
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            if (C[i][j] > 0){
               f_tmp[i][j] = f[i][j] + deltat*(f[min(i+1,width-1)][j] -2*f[i][j] + f[max(i-1,0)][j] 
               + f[i][min(j+1,height-1)] - 2*f[i][j] + f[i][max(j-1, 0)]);
            }
            else{
               f_tmp[i][j] = f[i][j];
            }
...
// writing picture
   ofstream outfile("6final.bmp", ios::out | ios::binary);
