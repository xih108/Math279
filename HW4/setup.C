...
// if creating picture
   ...
   while(true){
      for (i = 250; i <= 350; i++){
         for (j = 400; j <= 500; j++){
            f_tmp[i][j] = f[i][j] + deltat*(f[i+1][j] -2*f[i][j] + f[i-1][j] + f[i][j+1] - 2*f[i][j] + f[i][j-1]);
            if (abs(f_tmp[i][j]) > maxI){
               maxI = abs(f_tmp[i][j]);
            }
         }
      }
...
// writing picture
   ofstream outfile("5final.bmp", ios::out | ios::binary);
