...
void heat_flow(double **G, int width, int height, int steps);
...
// if reading in picture
   ifstream infile("objectnoisyblurry.bmp", ios::in | ios::binary);
   f = readpicture(infile,width,height);
   infile.close();
// if creating picture

   double **f_tmp = matrix(width, height);
   double **f_blur = matrix(width, height);
   ...
   double deltat =  epsilon*h*h / (4*wr + wf*epsilon*h*h / 2);
   ...
   for (int t = 0; t < numsteps; t++){
      ...
      // G*f 
      heat_flow(Gf, width, height, steps);
      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            Gf[i][j] -= f_blur[i][j];
         }
      ...
      for (i = 0; i < width; i++ ){
         for (j = 0; j < height; j++){
            double reg = 1/C_compute(f, i, j, width, height, epsilon) * (f[min(i+1, width-1)][j] - f[i][j] + f[i][min(j+1, height - 1)] - f[i][j])
                           - 1/C_compute(f, i-1, j, width, height, epsilon) * (f[i][j] - f[max(i-1, 0)][j])
                           - 1/C_compute(f, i, j-1, width, height, epsilon) * (f[i][j] - f[i][max(j-1, 0)]);
            double fit = Gf[i][j];
            f_tmp[i][j] = f[i][j] + deltat*( wr*reg - wf*fit);
         }
      }

      for (i = 0; i < width; i++){
         for (j = 0; j < height; j++){
            if (abs(f[i][j] - f_tmp[i][j]) > diff){
               diff = abs(f[i][j] - f_tmp[i][j]);
            }
            f[i][j] = f_tmp[i][j];
         }
      }  
   ...
   cout << "The value at pixel (53,53) is "<< f[53][53] << "."<< endl;
// writing picture
   ofstream outfile("5_final.bmp", ios::out | ios::binary);
...
void heat_flow(double **Gf, int width, int height, int steps){
   double** Gf_tmp = matrix(width, height);
   for (int k = 0; k < steps; k++){
      for (int i = 0; i < width; i++){
         for (int j = 0; j < height; j++){
            Gf_tmp[i][j] = (Gf[min(i+1, width-1)][j] + Gf[max(i-1, 0)][j] 
            + Gf[i][min(j+1, height-1)] + Gf[i][max(j-1,0)]) / 4.0;
         }
      }
