...
// if reading in picture
   ifstream infile("airplanenoisy.bmp", ios::in | ios::binary);
...
// if creating picture
   ...
   cout<<"Aaaa"<<endl;
   double r_k[height*width];
   double r_k1[height*width];
   ...
   while (true){
      ...
      double pA[width*height];
      memset(pA, 0.0, sizeof(pA));
      for (i = 0; i < width*height; i++){
         int row = i / height;
         int col = i % height;
      ...
      for (int i = 0; i < width*height; i++){
         ...
         for (int s = 0; s < 5; s++){
            int index = 5*i;
            Ap += val[index+s] * p[cols[s]];
         }
      
         // cout<<"Ap: "<<Ap<<endl;
         r_k1[i] = r_k[i] - alpha*Ap;
         if (abs(r_k1[i]) > diff){
            diff = abs(r_k1[i]);
         }
      }
...
// writing picture
   ofstream outfile("6c_final.bmp", ios::out | ios::binary);
