...
// if reading in picture
   ifstream infile("fruitnoisy.bmp", ios::in | ios::binary);
   f = readpicture(infile,width,height);
   infile.close();
// if creating picture

   double **f_tmp = matrix(width, height);
   double **f_noisy = matrix(width, height);
   ...
   while(true){
      ...
      for (i = 0; i < width; i++ ){
         for (j = 0; j < height; j++){
            double reg = C_compute(f, i, j, width, height, epsilon) * (f[min(i+1, width-1)][j] - f[i][j] + f[i][min(j+1, height - 1)] - f[i][j])
                           - C_compute(f, i-1, j, width, height, epsilon) * (f[i][j] - f[max(i-1, 0)][j])
                           - C_compute(f, i, j-1, width, height, epsilon) * (f[i][j] - f[i][max(j-1, 0)]);
            double fit = f[i][j] - f_noisy[i][j];
...
// writing picture
   ofstream outfile("1d_final.bmp", ios::out | ios::binary);
